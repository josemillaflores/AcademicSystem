...
builder.Services.AddMediatR(cfg => cfg.RegisterServicesFromAssembly(typeof(CreateEnrollmentCommandHandler).Assembly));
builder.Services.AddAutoMapper(typeof(EnrollmentServiceProfile));
builder.Services.AddDbContext<EnrollmentServiceDbContext>(options => options.UseNpgsql(builder.Configuration.GetConnectionString("DefaultConnection")));
builder.Services.AddScoped<IEnrollmentServiceRepository, EnrollmentServiceRepository>();
...
builder.Services.AddHealthChecks().AddDbContextCheck<EnrollmentServiceDbContext>("database");
